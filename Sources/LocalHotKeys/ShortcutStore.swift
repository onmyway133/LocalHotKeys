...
final class ShortcutStore {
    nonisolated(unsafe) static let shared = ShortcutStore()
    ...
    func modifiers(for id: String) -> NSEvent.ModifierFlags? {
        ...
        return NSEvent.ModifierFlags(rawValue: UInt(UserDefaults.standard.integer(forKey: modifiersKey(for: id))))
    }
    ...
    func reset(for id: String) {
        UserDefaults.standard.removeObject(forKey: keyCodeKey(for: id))
        UserDefaults.standard.removeObject(forKey: modifiersKey(for: id))
    }
}
